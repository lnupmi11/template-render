...
namespace REGEX
{
	...
	static const std::string FOREACH_REGEX("foreach\\s*\\(\\s*\\w+\\s+\\w+\\s*:?\\s*\\w+\\s*\\)");
	...
	static const std::string BEGIN_FOREACH_REGEX("\\{\\s*%\\s*" + FOREACH_REGEX + "\\s*%\\s*\\}");
	...
	static const std::string EMPTY_TAG("\\{\\s*%\\s*%\\s*\\}");
